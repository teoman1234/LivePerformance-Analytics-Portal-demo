...
export default function InfluencerPortal({ user, showPerformance = false }) {
  ...
  const influencerUser = useMemo(() => {
    const role = String(fallbackUser?.role || '').toLowerCase();
    if (role === 'influencer' || role === 'yayıncı' || role === 'yayinci') return fallbackUser;
    return null;
  }, [fallbackUser]);
  ...
  useEffect(() => {
    ...
    const pickFallbackUsername = async () => {
      try {
        const res = await fetch(api('/api/influencers?limit=1&sort=abps&order=desc'));
        if (!res.ok) return null;
        const data = await res.json();
        return data?.items?.[0]?.username || null;
      } catch {
        return null;
      }
    };
    ...
    const fetchData = async () => {
      try {
        ...
        if (!profileRes.ok) {
          const fallbackUsername = await pickFallbackUsername();
          if (!fallbackUsername) throw new Error('Could not load influencer data.');
          setOverrideUsername(fallbackUsername);
          return;
        }
